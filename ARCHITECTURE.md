# Architecture
This file provides information on how Cleanflower is structured, with descriptions of how each part of the codebase works.

## Terms
Cleanflower uses a large amount of terms and shorthand names within its code, for which explanations are provided here.
The first name is the full name and the names in parentheses are the shorthand names that are used throughout the code.
* Exprent (expr, exp) Refers to an expression that Cleanflower uses to create the java code, such as a constant or method invocation. Exprents are stored in a tree structure.
